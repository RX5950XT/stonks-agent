...
| P6.8 | `supply-chain`、release `build-scan` | `tests/policy/test_release_supply_chain.py`、`unsigned-supply-chain-candidate` | Supply-chain run `30194459983`已驗unsigned candidate；release run `30196542394`發布`v0.1.0` exact image後在Cosign v3重驗fail closed，未產生formal attestations或GitHub Release。 |
...
immutable releases；失敗的`v0.1.0` tag保持immutable且不得冒充formal，
正式`v0.1.1`成功前仍維持fail closed。
