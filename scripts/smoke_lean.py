#!/usr/bin/env python3
"""Send a deterministic canonical replay through a running LEAN sidecar."""
...
def _job(runtime_hash: str, image_digest: str, now: datetime) -> BacktestJob:
    ...
    return BacktestJob(
        ...
        attempt_nonce="lean-smoke-1",
        runtime=BacktestRuntimeIdentity(
            engine=BacktestEngineKind.LEAN,
            engine_version="17917+c22774e49ee80ecef5ca84f57616f6b66fad8bc5",
...
def _send(
    ...
    if payload.get("success") is not True:
        raise RuntimeError("LEAN smoke request failed")
...
def main() -> int:
    ...
    if first.semantic_hash != second.semantic_hash or tuple(
        ...
        raise RuntimeError("LEAN semantic or raw-reference replay changed")
    print(
        json.dumps(
            {
