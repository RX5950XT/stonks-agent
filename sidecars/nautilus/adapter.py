"""Canonical mapping boundary around the isolated NautilusTrader runtime."""
...
class AdapterPolicy:
    ...
    def __post_init__(self) -> None:
        if self.runtime.engine is not BacktestEngineKind.NAUTILUS:
            raise ValueError("Nautilus adapter requires a Nautilus runtime")
        if (
            self.max_orders < 1
            or self.max_bars < 1
            or self.max_order_bar_evaluations < 1
        ):
            raise ValueError("Nautilus adapter limits must be positive")
...
class NautilusAdapter:
    ...
    def run(self, job: BacktestJob) -> WorkerSuccess | WorkerFailure:
        ...
        try:
            ...
            return _failure("engine_unavailable", "Nautilus engine failed safely")
        if isinstance(trace, EngineFailure):
            return _failure(trace.code, trace.message)
        generated_at = self.clock()
        if generated_at > job.deadline:
            return _failure("deadline_expired", "Nautilus job deadline expired")
        try:
    ...
    def _preflight(self, job: BacktestJob) -> WorkerFailure | None:
        ...
        if (
            ...
            return _failure("job_too_large", "Nautilus job exceeds configured limits")
        now = self.clock()
        if now < job.requested_at:
            return _failure("job_not_ready", "Nautilus job is not ready")
        if now > job.deadline:
            return _failure("deadline_expired", "Nautilus job deadline expired")
        return None
...
def compute_runtime_hash(root: Path) -> str:
    ...
    for name in _RUNTIME_FILES:
        path = root / name
        if not path.is_file():
            raise ValueError(f"Nautilus runtime file is missing: {name}")
...
def _map_result(
    ...
    *,
    ...
    return BacktestResult.create(
        result_id=uuid5(
            NAMESPACE_URL,
            f"stonks-nautilus-result:{job.job_hash}:{trace_hash}:{generated_at.isoformat()}",
        ),
        job=job,
...
def _validate_trace(job: BacktestJob, trace: EngineRunTrace) -> None:
    ...
    if set(actual) != expected or len(actual) != len(set(actual)):
        raise ValueError("Nautilus order trace coverage changed")
    if len(fill_ids) != len(set(fill_ids)) or any(not item for item in fill_ids):
        raise ValueError("Nautilus fill trace IDs are invalid")
    if any(
        ...
        raise ValueError("Nautilus fill trace is invalid")
...
def _map_fill(
    ...
    if price is None:
        raise ValueError("Nautilus fill has no canonical price")
    fee = canonical_fill_fee(trace.quantity, price, job.cost_model)
    fill_id = uuid5(
        NAMESPACE_URL,
        f"stonks-nautilus-fill:{job.input_hash}:{trace.order_id}:{trace.source_bar_id}:{trace.external_fill_id}",
    )
    return BacktestFill.create(
        ...
        external_ref=(
            f"nautilus:{trace.external_fill_id}:raw-sha256:{trace.raw_event_hash}"
        ),
    )
...
def _map_outcomes(
    ...
    for order in job.orders:
        ...
        if filled > order.quantity:
            raise ValueError("Nautilus filled quantity exceeds command")
