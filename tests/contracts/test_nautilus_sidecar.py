...
from sidecars.nautilus.app import create_app  # noqa: E402
...
def adapter(response: EngineRunTrace | EngineFailure) -> NautilusAdapter:
    request = job(BacktestEngineKind.NAUTILUS)
...
def test_adapter_maps_complete_trace_to_canonical_replay() -> None:
    ...
    assert first.value.fills[0].external_ref == (
        "nautilus:T-001:raw-sha256:" + "c" * 64
    )
...
def test_http_surface_is_bounded_typed_and_has_no_execution_route() -> None:
    ...
    assert health.json()["data"]["engine"] == "nautilus"
